#!/usr/bin/env python3
"""
8-Hour Multi-Timeframe CipherB Analyzer - FIXED VERSION
- Enhanced symbol mapping for BingX/KuCoin
- Freshness validation (8 hours)
...
class Analyzer8h:
    ...
    def load_market_data(self):
        ...
        print(f"📊 Loaded {len(coins)} coins for 8h analysis")
        return coins
    ...
    def fetch_ohlcv_data(self, symbol, timeframe='8h'):
    ...
    def analyze_coin(self, coin_data):
        """Analyze single coin for 8h CipherB signals with 8-hour freshness"""
        symbol = coin_data['symbol']
        
        try:
            df, exchange_used = self.fetch_ohlcv_data(symbol, '8h')
            ...
            # ✅ FRESHNESS CHECK: Signal must be within last 8 hours
            if not is_signal_fresh(signal_timestamp_utc, '8h'):
                signal_age_display = get_signal_age_display(signal_timestamp_utc, '8h')
    ...
    def run_analysis(self):
        """Run 8h CipherB analysis"""
        ist_current = get_ist_time()
        
        print("="*80)
        print("🎯 8-HOUR MULTI-TIMEFRAME ANALYSIS")
        print("="*80)
        ...
        print(f"⏰ Freshness: Signals within last 8 hours only")
        ...
        for i in range(0, len(self.market_data), batch_size):
            ...
            for coin in batch:
                ...
                if signal_result:
                    ...
                    print(f"🚨 FRESH 8H {signal_result['signal_type']}: {signal_result['symbol']} "
                          f"({age_s:.0f}s ago)")
