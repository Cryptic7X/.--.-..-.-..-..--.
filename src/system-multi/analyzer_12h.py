#!/usr/bin/env python3
"""
Multi-Timeframe 12H CipherB Analysis System
- Pure CipherB signals (no confirmation needed)
- Highest timeframe in cascade: always allows signals
- Final stage in multi-timeframe suppression logic
"""
...
class Multi12HAnalyzer:
    ...
    def load_blocked_coins(self):
        ...
        try:
            ...
            print(f"🚫 Loaded {len(blocked_coins)} blocked coins for 12h analysis")
    ...
    def analyze_coin_for_signals(self, coin_data):
        """
        Analyze single coin for pure CipherB signals on 12h timeframe
        12h is highest timeframe - always allows valid signals
        """
        ...
        try:
            ...
            # Apply advanced suppression logic (12h is always allowed if signal exists)
    ...
    def run_analysis(self):
        """
        Run complete 12-hour multi-timeframe analysis
        """
        start_time = time.time()
        ist_time = get_ist_time()
        
        print("="*80)
        print("📈 MULTI-TIMEFRAME 12H CIPHERB ANALYSIS")
        print("="*80)
        ...
        print(f"🔄 Suppression: Highest timeframe (always allowed)")
        print(f"🎯 GitHub Actions Run: #{os.getenv('GITHUB_RUN_NUMBER', 'local')}")
        
        # Validate analysis timing
        timing_valid, timing_reason = is_analysis_time_valid('12h', ist_time)
        if not timing_valid:
            print(f"⏸️ TIMING CHECK: {timing_reason}")
            send_admin_alert("12h Analysis Timing Issue", f"Reason: {timing_reason}\nTime: {ist_time}")
        ...
        try:
            ...
            if not market_data:
                error_msg = "❌ No market data available for 12h analysis"
                print(error_msg)
                send_admin_alert("12h Analysis Failed", error_msg)
                return
            ...
            if valid_signals:
                success = send_multi_consolidated_alert(valid_signals, timeframe='12h')
                if success:
                    print(f"\n📱 12H ALERT SENT: {len(valid_signals)} signals")
                else:
                    print(f"\n❌ 12H Alert delivery failed")
                    send_admin_alert("12h Alert Failed", f"Failed to send {len(valid_signals)} signals")
            else:
                print(f"\n📊 No valid 12h CipherB signals detected")
            ...
            print(f"\n" + "="*80)
            print("📈 12-HOUR ANALYSIS COMPLETE")
            print("="*80)
            ...
            error_msg = f"Critical error in 12h analysis: {str(e)}"
            print(f"💥 {error_msg}")
            print(f"📋 Traceback: {traceback.format_exc()}")
            send_admin_alert("12h Analysis Critical Error", f"{error_msg}\n\n{traceback.format_exc()}")
    ...
    def save_analysis_cache(self, signals, total_analyzed, start_time):
        ...
        try:
            cache_data = {
                ...
                'system': 'multi-timeframe-12h',
                'version': self.config['system']['version']
            }
            
            cache_file = self.config['cache']['analysis_cache_12h']
            ...
            print(f"⚠️ Failed to save 12h analysis cache: {e}")

if __name__ == '__main__':
    analyzer = Multi12HAnalyzer()
