#!/usr/bin/env python3
"""
Multi-Timeframe 8H CipherB Analysis System
- Pure CipherB signals (no confirmation needed)
- Advanced suppression logic with timeframe cascading
- Inherits 8h position in cascade: waits for 6h suppression
"""
...
class Multi8HAnalyzer:
    ...
    def load_blocked_coins(self):
        ...
        try:
            ...
            print(f"🚫 Loaded {len(blocked_coins)} blocked coins for 8h analysis")
    ...
    def analyze_coin_for_signals(self, coin_data):
        """
        Analyze single coin for pure CipherB signals on 8h timeframe
        Implements 8h position in cascade logic
        """
        ...
        try:
            ...
            # Apply advanced suppression logic (8h cascade position)
    ...
    def run_analysis(self):
        """
        Run complete 8-hour multi-timeframe analysis
        """
        start_time = time.time()
        ist_time = get_ist_time()
        
        print("="*80)
        print("📈 MULTI-TIMEFRAME 8H CIPHERB ANALYSIS")
        print("="*80)
        print(f"🕐 Analysis Time: {ist_time.strftime('%Y-%m-%d %H:%M:%S IST')}")
        print(f"⏰ Timeframe: 8-hour candles")
        print(f"🎯 Signal Type: Pure CipherB (no confirmation)")
        print(f"🔄 Suppression: 8h cascade position")
        print(f"🎯 GitHub Actions Run: #{os.getenv('GITHUB_RUN_NUMBER', 'local')}")
        
        # Validate analysis timing
        timing_valid, timing_reason = is_analysis_time_valid('8h', ist_time)
        if not timing_valid:
            print(f"⏸️ TIMING CHECK: {timing_reason}")
            send_admin_alert("8h Analysis Timing Issue", f"Reason: {timing_reason}\nTime: {ist_time}")
        ...
        try:
            ...
            if not market_data:
                error_msg = "❌ No market data available for 8h analysis"
                print(error_msg)
                send_admin_alert("8h Analysis Failed", error_msg)
                return
            ...
            for i in range(0, len(filtered_coins), batch_size):
                ...
                for coin in batch:
                    ...
                    if signal_result:
                        ...
                        print(f"✅ 8H {signal_result['signal_type']}: {signal_result['symbol']} "
            ...
            if valid_signals:
                success = send_multi_consolidated_alert(valid_signals, timeframe='8h')
                if success:
                    print(f"\n📱 8H ALERT SENT: {len(valid_signals)} signals")
                else:
                    print(f"\n❌ 8H Alert delivery failed")
                    send_admin_alert("8h Alert Failed", f"Failed to send {len(valid_signals)} signals")
            else:
                print(f"\n📊 No valid 8h CipherB signals detected")
            ...
            print(f"\n" + "="*80)
            print("📈 8-HOUR ANALYSIS COMPLETE")
            print("="*80)
            ...
            print(f"⏰ Next Analysis: 8 hours")
            print("="*80)
            
        except Exception as e:
            error_msg = f"Critical error in 8h analysis: {str(e)}"
            print(f"💥 {error_msg}")
            print(f"📋 Traceback: {traceback.format_exc()}")
            send_admin_alert("8h Analysis Critical Error", f"{error_msg}\n\n{traceback.format_exc()}")
    ...
    def save_analysis_cache(self, signals, total_analyzed, start_time):
        ...
        try:
            cache_data = {
                ...
                'system': 'multi-timeframe-8h',
                'version': self.config['system']['version']
            }
            
            cache_file = self.config['cache']['analysis_cache_8h']
            ...
            print(f"⚠️ Failed to save 8h analysis cache: {e}")

if __name__ == '__main__':
    analyzer = Multi8HAnalyzer()
